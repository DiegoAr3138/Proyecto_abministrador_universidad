...
def Crear_base_de_datos():
    ...
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS ADMINISTRATIVO (ID INTEGER PRIMARY KEY AUTOINCREMENT, USUARIO VARCHAR, CONTRASEÑA VARCHAR)")
...
def administrativo():
    def profesor():
        def crearProfesor():
            ...
            def crear():
                ...
                conexion = sqlite3.connect('Base_total')
                cursor = conexion.cursor()
                cursor.execute("INSERT INTO PROFESORES VALUES (NULL,?,?,?,0)",(nombreTotal , MateriaAsignada.get(), contrase.get()))
                conexion.commit()
                cursor.close()
                conexion.close()
            ...
            Entry(vcrearprofesor, width=20, textvariable=contrase).grid(row=3,column=1)
        ...
        def editarProfesor():
            def verificar():
                global encontrar, encontra_profesor
                conexion = sqlite3.connect('Base_total')
                cursor = conexion.cursor()
                cursor.execute("SELECT * FROM PROFESORES")
                lista_profesores= cursor.fetchall()
                ...
                if encontrar:
                    ...
                    contrase.set(encontra_profesor[3])
            ...
            def guardar():
                if encontrar:
                    ...
                    conexion = sqlite3.connect('Base_total')
                    cursor = conexion.cursor()
                    cursor.execute("UPDATE PROFESORES SET NOMBRE='%s', MATERIAS='%s', CONTRASEÑA='%s' WHERE CODIGO='%s'"%(nombre.get(),MateriaAsignada.get(),contrase.get(),int(Id.get())))
                    conexion.commit()
                    cursor.close()
                    conexion.close()
            ...
            Entry(vEditarProfesor, textvariable=contrase).grid(row=3,column=1)
    ...
    def estudiantes():
        ...
        def crearEstudiante():
            ...
            def crear():
                ...
                conexion = sqlite3.connect('Base_total')
                cursor = conexion.cursor()
                cursor.execute("INSERT INTO ESTUDIANTES VALUES (NULL,?,?,?,0,?,0)",(nombreTotal , eleccion.get(), contrase.get(), materias.get()))
                conexion.commit()
                cursor.close()
                conexion.close()
            ...
            Entry(vCrearEstudiante, width=20, textvariable=contrase).grid(row=3,column=1)
        ...
        def editarEstudiante():
            def verificar():
                global encontrar, encontra_estudiante
                conexion = sqlite3.connect('Base_total')
                cursor = conexion.cursor()
                cursor.execute("SELECT * FROM ESTUDIANTES")
                lista_estudiantes= cursor.fetchall()
                ...
                if encontrar:
                    ...
                    contrase.set(encontra_estudiante[3])
            ...
            def guardar():
                if encontrar:
                    ...
                    conexion = sqlite3.connect('Base_total')
                    cursor = conexion.cursor()
                    cursor.execute("UPDATE ESTUDIANTES SET NOMBRE='%s', CARRERA='%s', CONTRASEÑA='%s', MATERIAS='%s' WHERE CODIGO='%s'"%(nombre.get(),eleccion.get(),contrase.get(),materias.get(),int(Id.get())))
                    conexion.commit()
                    cursor.close()
                    conexion.close()
                    messagebox.showinfo('Editado correctamente', 'Profesor editado correctamente')
            ...
            Entry(vEditarEstudiante, width=20, textvariable=contrase).grid(row=3,column=1)
    ...
    def administradores():
        ...
        def crearAdministrador():
            def verificarA():
                if len(usuario.get())>0 and len(contrase.get())>0:
                    guardar()
                else:
                    messagebox.showinfo('Error', 'Por favor llene las casillas')
                    crearAdmi.destroy()
            ...
            def guardar():
                conexion=sqlite3.connect('Base_total')
                cursor=conexion.cursor()
                cursor.execute("INSERT INTO ADMINISTRATIVO VALUES(NULL,?,?)",(usuario.get(), contrase.get()))
                conexion.commit()
                cursor.close()
                conexion.close()
            ...
            Entry(crearAdmi, textvariable=contrase).grid(row=3, column=1)
        ...
        def editarAdmi():
            ...
            Entry(editarAdministrador, textvariable=contrase).grid(row=3, column=1)
    ...
    def matricula():
        ...
        def fmostrar(ventedit, buscar, busgrado):
            ...
            if permi == 0:
                ...
                Button(vnuevo, text='ELIMINAR', command=lambda: feliminar2(vnuevo, buscgrado, busqueda), font=(30),
                       bg='#56cd63',
        ...
        def feliminar2(borrar, buscgrado, busqueda):
            qq = borrar
            ruloz = sqlite3.connect("matriculas.db")
            cursor = ruloz.cursor()
            if buscgrado == '1':
...
def inisiar_secionU(Usuario, Contrase, tipov):
    conexion=sqlite3.connect("Base_total")
    micursor=conexion.cursor()
    if tipov=="Estudiantes":
        micursor.execute("SELECT * FROM ESTUDIANTES WHERE CODIGO=? AND CONTRASEÑA=?", (Usuario, Contrase))
        if micursor.fetchall():
            votoe=0
            micursor.execute("SELECT VOTO FROM ESTUDIANTES WHERE CODIGO=? AND CONTRASEÑA=? AND VOTO=?", (Usuario, Contrase, votoe))
            if micursor.fetchall():
                votacionesE(Usuario)
        ...
        micursor.execute("SELECT * FROM PROFESORES WHERE CODIGO=? AND CONTRASEÑA=?", (Usuario, Contrase))
        if micursor.fetchall():
            votop=0
            micursor.execute("SELECT VOTO FROM PROFESORES WHERE CODIGO=? AND CONTRASEÑA=? AND VOTO=?", (Usuario, Contrase, votop))
            if micursor.fetchall():
                votacionesP(Usuario)
...
def texto(Textusu,TextCon,tipov):
    Usuario=Textusu.get()
    Contrase=TextCon.get()
    Textusu.delete(0,END)
    TextCon.delete(0,END)
    inisiar_secionU(Usuario,Contrase,tipov)
...
def ventanaprim():
    ...
    def admin():
        ...
        def inicio():
            ...
            TituloCon=Label(vinicio,text="Contrase:", font=("Times New Roman",16))
            TituloCon.grid(row=3, column=0,sticky="w",padx=50)
        ...
        def inicio_seciona(Textusu,TextCon):
            ...
            conexion=sqlite3.connect("Base_total")
            micursor=conexion.cursor()
            micursor.execute("SELECT * FROM ADMINISTRATIVO WHERE USUARIO=? AND CONTRASEÑA=?",(Usuario,Contrase))
...
def AdrianaR():
    ...
    def login_sucess():
        ...
        Button(login_success_screen, text='Datos', command=lambda: (login_success_screen.destroy(), editar())) \
    ...
    def editar():
        global segundo_nombre_entry, primer_apellido_entry, segundo_apellido_entry
        edit_screen = Tk()
        edit_screen.geometry("222x230")
        ...
        Label(edit_screen, text="Codigo").pack()
        ...
        Label(edit_screen, text="Nombre Completo").pack()
        ...
        Label(edit_screen, text="Carrera").pack()
        ...
        Label(edit_screen, text="").pack()
        Button(edit_screen, text="Salir", width=10, height=1, command=lambda: (edit_screen.destroy())).pack()
    ...
    def main():
        ...
        Label(main_screen, text="").pack()
        Button(main_screen, text="Login", width=10, height=1, command=lambda: (login())).pack()
...
def preguntasM():#multiple
    ...
    def guardar():
        ...
        cursor = coneccion.cursor()
        cursor.execute(
            "INSERT INTO ENCUESTAS(ENCUESTA,NOMBRE,PREGUNTA1,PREGUNTA2,PREGUNTA3,PREGUNTA4,PREGUNTA5,PREGUNTA6,PREGUNTA7,PREGUNTA8,PREGUNTA9,PREGUNTA10,TIPO) VALUES (NULL,?,?,?,?,?,?,?,?,?,?,?,1)",
            (nombre.get(), preguntas.get(), preguntas2.get(), preguntas3.get(), preguntas4.get(), preguntas5.get(),
             preguntas6.get(), preguntas7.get(), preguntas8.get(), preguntas9.get(), preguntas10.get()))
        coneccion.commit()
        cursor.close()
        coneccion.close()
    ...
    Button(crear, text="Crear", command=lambda: (guardar(), crear.destroy(), CrearEncuestas())).grid(row=11, column=1,
                                                                                                     pady=10, padx=10)
    Button(crear, text="Volver", command=lambda: (crear.destroy(), CrearEncuestas())).grid(row=11, column=0, pady=10,
                                                                                           padx=10)
    crear.mainloop()
...
def preguntasA():#abierta
    ...
    def guardar():
        ...
        cursor = coneccion.cursor()
        cursor.execute(
            "INSERT INTO ENCUESTAS(ENCUESTA,NOMBRE,PREGUNTA1,PREGUNTA2,PREGUNTA3,PREGUNTA4,PREGUNTA5,PREGUNTA6,PREGUNTA7,PREGUNTA8,PREGUNTA9,PREGUNTA10,TIPO) VALUES (NULL,?,?,?,?,?,?,?,?,?,?,?,2)",
            (nombre.get(), preguntas.get(), preguntas2.get(), preguntas3.get(), preguntas4.get(), preguntas5.get(),
             preguntas6.get(), preguntas7.get(), preguntas8.get(), preguntas9.get(), preguntas10.get()))
        coneccion.commit()
        cursor.close()
...
def ResponderEncuestas():
    ...
    def verificacion():
        ...
        global encontrarENCUESTA, encuesta_encontrada
...
class notas:
    ...
    def materia(self):
        ...
        Label(self.win, text=self.m, fg='blue', font=("Courier", 16, "italic")) \
        ...
        Button(self.win, text='Editar notas', fg='red', font=("Comic Sans MS", 12, "italic"),
               command=lambda: self.editnotas()).grid(row=2, column=1)
        Button(self.win, text='Volver', fg='red', font=("Comic Sans MS", 12, "italic"),
               command=lambda: self.materias()).grid(row=2, column=1, sticky='w')
    ...
    def editnotas(self):
        ...
        Button(self.win1, text='Guardar Notas', fg='red', font=("Comic Sans MS", 12, "italic"),
               command=lambda: self.guardarnotas()).grid(row=6, column=1)
...
if len(lista_admi)==0:
    ...
    Entry(raizs, textvariable=contrase).grid(row=3, column=1)
