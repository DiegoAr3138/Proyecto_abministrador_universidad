...
def administrativo():
	...
	def CargarMaterias():
		...
		conexion = sqlite3.connect('Base_total')
		cursor = conexion.cursor()
		cursor.execute("""CREATE TABLE IF NOT EXISTS NOTAS (CODIGO INTEGER ,
					NOMBRE VARCHAR,
					CARRERA VARCHAR,
					SABER VARCHAR,
					SABERHACER INTEGER,
...
class notas:
    ...
    def conexion(self, query, parametros=()):
        ...
        with sqlite3.connect('Base_total') as conn:
            ...
            cursor.execute("""CREATE TABLE IF NOT EXISTS NOTAS (CODIGO INTEGER ,NOMBRE VARCHAR,CARRERA VARCHAR,
                            SABER VARCHAR,SABERHACER VARCHAR,SER VARCHAR,MATERIA)""")
    ...
    def guardarnotas(self):
        try:
            ...
            consulta = 'UPDATE NOTAS SET SABER=?,SABERHACER=?,SER=? WHERE SABER=? AND SABERHACER=? AND SER=? AND CODIGO=? AND MATERIA=?'
...
def inicioP():
	...
	estudianteMenu.add_command(label='Ingresar',command=lambda:(AdrianaR()))
